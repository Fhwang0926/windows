...
configureDNS() {
  ...
  # dnsmasq configuration with unique PID file:
  DNSMASQ_PID="/var/run/dnsmasq_$$.pid"
  DNSMASQ_OPTS="$DNSMASQ_OPTS --pid-file=$DNSMASQ_PID --dhcp-range=${VM_NET_IP%.*}.2,${VM_NET_IP%.*}.10 --dhcp-host=$VM_NET_MAC,,$VM_NET_IP,$VM_NET_HOST,infinite --dhcp-option=option:netmask,255.255.255.0"
...
configureNAT() {
  ...
  # Create a bridge with a static IP for the VM guest (unique per container)
  VM_NET_IP="20.20.20.$((20 + ($$ % 10)))"
  # VM_NET_HOST='127.0.1.1'
  
  # Generate unique bridge name to avoid conflicts (simplified for SELinux)
  BRIDGE_NAME="br$(echo $$ | tail -c 4)"
  info "Using bridge name: $BRIDGE_NAME"

  # Check if bridge already exists
  if ip link show "$BRIDGE_NAME" >/dev/null 2>&1; then
    info "Bridge '$BRIDGE_NAME' already exists, reusing it"
    rc=0
  else
    { ip link add dev "$BRIDGE_NAME" type bridge ; rc=$?; } || :
  fi
  ...
  # Add IP address to bridge if not already configured
  if ! ip address show "$BRIDGE_NAME" | grep -q "${VM_NET_IP%.*}.1/24"; then
    ip address add ${VM_NET_IP%.*}.1/24 broadcast ${VM_NET_IP%.*}.255 dev "$BRIDGE_NAME"
  else
    info "IP address ${VM_NET_IP%.*}.1/24 already configured on $BRIDGE_NAME"
  fi

  while ! ip link set "$BRIDGE_NAME" up; do
    info "Waiting for IP address to become available..."
    sleep 2
  done

  # QEMU Works with taps, set tap to the bridge created
  # Generate unique TAP name to avoid conflicts (simplified for SELinux)
  VM_NET_TAP="tap$(echo $$ | tail -c 4)"
  info "trying $VM_NET_TAP"
  
  # Check if TAP already exists
  if ip link show "$VM_NET_TAP" >/dev/null 2>&1; then
    info "TAP '$VM_NET_TAP' already exists, reusing it"
  else
...
closeNetworkCustom() {
  ...
  # 브리지 정리 (안전하게)
  if ip link show "$BRIDGE_NAME" >/dev/null 2>&1; then
    # 브리지에 연결된 인터페이스가 있는지 확인
    if ! ip link show "$BRIDGE_NAME" | grep -q "master $BRIDGE_NAME"; then
      info "Cleaning up bridge: $BRIDGE_NAME"
      ip link set "$BRIDGE_NAME" down || true
      ip link delete "$BRIDGE_NAME" || true
    else
      info "Bridge $BRIDGE_NAME is still in use, skipping cleanup"
    fi
  fi
...
configureSMBLocal () {
  ...
  if [[ "$NFS_LOCAL" == [Yy1]* ]]; then
    ...
    info "starting wsdd"
    # Use unique port for wsdd to avoid conflicts
    WSDD_PORT=$((3702 + ($$ % 100)))
    wsdd -i "$BRIDGE_NAME" -p "$WSDD_PORT" -n "host.lan" & 
    info "started wsdd on port $WSDD_PORT"
